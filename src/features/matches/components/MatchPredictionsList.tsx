...
export function MatchPredictionsList({ matchId }: MatchPredictionsListProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white/5 border border-white/10 rounded-app overflow-hidden min-h-[300px] flex flex-col">
        {loading ? (
          ...
          <div className="divide-y divide-white/5">
            {predictions.map((prediction) => {
              ...
              return (
                <div key={prediction.id} className="p-4 flex items-center justify-between hover:bg-white/5 transition-colors group">
                  <div className="flex items-center gap-3">
                    {/* Avatar */}
                    <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-white/10 overflow-hidden relative border border-white/10">
                         {/* Placeholder or nothing since we don't have avatar yet in User type visible in explore, assuming initials or similar if needed. 
                             Actually the User type usually has avatar? Let's check user object in code.
                             The viewed User type didn't explicitly show avatar field? Wait, let me check view_file 21.
                             Ah, User collection DOES NOT have avatar field... it mentions badges, jersey etc. 
                             Wait, check User type again. 
                             I'll assume no avatar for now and use initials or jersey colors if available.
                             The User has `jersey` field. I can use that for a cool avatar.
                         */}
                         <div 
                           className="w-full h-full flex items-center justify-center font-black text-[10px] md:text-xs text-secondary-foreground"
                           style={{ backgroundColor: user.jersey?.primaryColor || '#333', color: user.jersey?.secondaryColor || '#fff' }}
                         >
                      ...
                      <div className="flex items-center gap-2">
                         ...
                         {user.badges && user.badges.length > 0 && (
                            <div className="px-1.5 py-0.5 rounded bg-white/10 border border-white/5 text-[8px] font-bold text-white/60">
                               {/* Just a badge count or icon for now */}
                  ...
                  <div className="flex items-center gap-4 md:gap-8">
                    ...
                    <div className="w-16 text-right">
                       {(prediction.points !== null || (prediction as any).potentialPoints !== undefined) && (
