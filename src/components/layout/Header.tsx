...
import { Menu, Book, Settings, Users, CheckCircle, ChevronDown, Trophy, Loader2 } from 'lucide-react'
  ...
  const handleLeagueChange = async (leagueId: string | 'global') => {
    ...
    if (slug) {
        try {
            ...
            await updateActiveLeagueAction(
                slug, 
                newValue
            )
        } catch (err) {
              ...
              <Dialog>
                  <DialogTrigger asChild>
                    <Button
                        variant="ghost"
                        className={cn(
                          "h-9 px-4 rounded-md border transition-all cursor-pointer group flex items-center gap-3",
                          effectiveLeagueId 
                    ...
                    >
                        <div className={cn(
                          ...
                          {effectiveLeagueId ? <Users className="w-3 h-3" /> : <Trophy className="w-3 h-3" />}
                        </div>
                        <span className={cn(
                   ...
                   <div className="grid gap-2 py-4">
                      ...
                      <DialogClose asChild>
                         <button
                            ...
                            className={cn(
                               "flex items-center justify-between p-4 rounded-xl border transition-all cursor-pointer group w-full",
                               !effectiveLeagueId
                      ...
                      {leagues.map(league => (
                         <DialogClose key={league.id} asChild>
                            ...
                            >
                               ...
                               {effectiveLeagueId === league.id && <CheckCircle className="w-5 h-5 text-[#eab308]" />}
                            </button>
                         </DialogClose>
                      ))}
              ...
              <div className="flex flex-col gap-6 mt-8">
                 ...
                 {slug && (
                    <div className="flex flex-col gap-2 p-4 bg-white/5 rounded-xl border border-white/10">
                       <span className="text-xs font-black uppercase tracking-widest text-white/40 mb-2">Liga</span>
                       <div className="grid grid-cols-1 gap-2">
                               <Button
                                  ...
                                  className={cn(
                                    ...
                                    !effectiveLeagueId ? "bg-[#eab308] text-black" : "text-white/60 bg-white/5"
                                  )}
                               >
                                  Globálna
                               </Button>
                               {leagues.map(league => (
                                <Button
                                  ...
                                  onClick={() => handleLeagueChange(league.id)}
                                   className={cn(
                                    "justify-start text-xs font-bold uppercase tracking-wider",
                                    effectiveLeagueId === league.id ? "bg-[#eab308] text-black" : "text-white/60 bg-white/5"
                                  )}
                                >
                                   {league.name}
                                </Button>
